...
class TestParseSpecHeader:
    def test_real_header_contains_required_createc_fields_with_physical_ranges(self):
        ...
        assert 0 < float(hdr["Dacto[A]xy"]) < 1.0
...
class TestSyntheticDACConversion:
    ...
    def test_synthetic_dac_scale_and_current_sign_contract(self, tmp_path):
        spec = read_spec_file(_make_synthetic_vert(tmp_path, z_dac=100.0, i_dac=-5000.0))
        ...
        positive = read_spec_file(_make_synthetic_vert(tmp_path, z_dac=0.0, i_dac=3000.0))
        assert positive.channels["I"].mean() > 0
...
class TestChannelMetadata:
    def test_real_fixture_channel_order_and_defaults_are_valid(self, bias_sweep_spec, time_trace_spec):
        for spec in (bias_sweep_spec, time_trace_spec):
            assert spec.channel_order == ["I", "Z", "V"]
            assert spec.default_channels
            assert all(ch in spec.channels for ch in spec.default_channels)

    def test_additional_decoded_channels_reach_specdata_and_metadata(self, tmp_path):
        f = _write_createc_vert(
            tmp_path,
            "extra_channels.VERT",
            _createc_vert_text(
