"""Grain and periodicity analysis: emit measurements, not transformed arrays.

Moved out of :mod:`probeflow.processing.analysis` (review arch-backend #15).
These three helpers produce measurements (auto-clip percentiles, grain label
maps + statistics, periodogram peaks) rather than processed image arrays, so
they belong in ``analysis/`` per the canonical
``Scan → ProcessingState → ProcessingHistory + ExportRecord`` layering.

``processing/analysis.py`` continues to re-export the public names so the
historical ``probeflow.processing.gmm_autoclip`` / ``.detect_grains`` /
``.measure_periodicity`` import surface remains stable.
"""
...
# ═════════════════════════════════════════════════════════════════════════════
# gmm_autoclip  (UniMR: GMM auto-thresholding)
# ═════════════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════════════
# detect_grains  (Gwyddion: "Mark Grains by Threshold / Watershed")
# ═════════════════════════════════════════════════════════════════════════════
