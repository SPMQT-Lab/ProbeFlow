...
_MODE_BUTTONS: list[tuple[str, str, str]] = [
    ("pan",       "Pan",     "Navigate the image by dragging. Does not create ROIs."),
    ("rectangle", "Rect",    "Draw a rectangular ROI for processing or measurement.  [R]"),
    ("ellipse",   "Ellipse", "Draw an elliptical ROI for processing or measurement.  [E]"),
    ("polygon",   "Poly",    "Draw a polygon ROI by clicking vertices.  [P]"),
    ("freehand",  "Free",    "Draw a freehand ROI.  [F]"),
    ("line",      "Line",    "Draw a line ROI for profiles, step heights, and periodicity.  [L]"),
    ("point",     "Point",   "Place a point ROI.  [T]"),
]

_ACTION_BUTTONS: list[tuple[str, str, str]] = [
    ("auto_contrast",    "Auto",      "Autoscale image contrast using the current display settings."),
    ("plane_background", "Plane",     "Open simple plane/background subtraction."),
    ("stm_background",   "STM BG",   "Open STM scan-line background subtraction."),
    ("bad_lines",        "Bad lines", "Open bad scan-line correction."),
    ("open_fft",         "FFT",       "Open the FFT viewer for the current image."),
    ("open_lattice_grid","Grid",      "Open the lattice/grid measurement tool."),
    ("line_periodicity", "Period",    "Estimate periodicity from the selected line ROI."),
    ("line_profile",     "Profile",   "Show a line profile for the selected line ROI."),
]

_ROI_ACTION_BUTTONS: list[tuple[str, str, str]] = [
    ("mask_selection",   "Mask",   "Create or apply a mask from the current ROI/selection."),
    ("invert_selection", "Invert", "Invert the current area ROI or mask."),
]
...
class ImageQuickToolbar(QWidget):
    ...
    def _build_mode_row(self) -> QHBoxLayout:
        ...
        for key, label, tip in _MODE_BUTTONS:
            btn = self._make_btn(label, tip, checkable=True)
        ...
        clear_btn = self._make_btn("Clear", "Clear the active ROI or current selection.")
    ...
    def _build_action_row(self) -> QHBoxLayout:
        ...
        for key, label, tip in _ACTION_BUTTONS:
            btn = self._make_btn(label, tip)
    ...
    def _build_roi_row(self) -> QHBoxLayout:
        ...
        for key, label, tip in _ROI_ACTION_BUTTONS:
            btn = self._make_btn(label, tip)
            btn.clicked.connect(lambda _checked=False, k=key: self.action_requested.emit(k))
    ...
    @staticmethod
    def _make_btn(label: str, tip: str, *, checkable: bool = False) -> QPushButton:
