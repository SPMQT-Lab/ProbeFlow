...
class DatasetBuilderFolderIndexSignals(QObject):
    indexed = Signal(object, object, object)  # source, indexed_items, token
    failed = Signal(object, str, object)      # source, message, token


class DatasetBuilderFolderIndexLoader(_PooledWorker):
    """Run the Dataset Builder folder scan off the GUI thread."""

    def __init__(self, source, token) -> None:
        super().__init__(DatasetBuilderFolderIndexSignals())
    ...
    def work(self):
        ...
        try:
            if self.source.is_file():
                ...
                items = index_folder(self.source, recursive=True, include_errors=True)
        except Exception as exc:
            _log.warning("DatasetBuilderFolderIndexLoader: failed to index %s (%s)", self.source, exc)
            self.signals.failed.emit(self.source, str(exc), self.token)
            return
        self.signals.indexed.emit(self.source, items, self.token)


class DatasetBuilderQueueHydrateSignals(QObject):
    loaded = Signal(object, object, object)  # source, queue_items, token
    failed = Signal(object, str, object)     # source, message, token


class DatasetBuilderQueueHydrateWorker(_PooledWorker):
    """Convert indexed scan items into Dataset Builder queue rows off-thread."""

    def __init__(self, source, indexed_items, task, label_name, plane_index, token) -> None:
        super().__init__(DatasetBuilderQueueHydrateSignals())
        self.source = Path(source)
        self.indexed_items = list(indexed_items)
        self.task = task
    ...
    def work(self):
        try:
            queue = build_queue_from_indexed_items(
                self.indexed_items,
                task=self.task,
            ...
            _log.warning("DatasetBuilderQueueHydrateWorker: failed for %s (%s)", self.source, exc)
            self.signals.failed.emit(self.source, str(exc), self.token)
            return
        self.signals.loaded.emit(self.source, queue, self.token)


class DatasetBuilderSampleLoadSignals(QObject):
    loaded = Signal(object, object, object)  # key, LoadedSampleRaw, token
    failed = Signal(object, str, object)     # key, message, token


class DatasetBuilderSampleLoadWorker(_PooledWorker):
    """Load one scan plane off-thread for the active Dataset Builder sample."""

    def __init__(self, path, plane_index: int, flatten_enabled: bool, token) -> None:
        super().__init__(DatasetBuilderSampleLoadSignals())
        self.path = Path(path)
        self.plane_index = int(plane_index)
    ...
    def work(self):
        ...
        key = sample_cache_key(self.path, self.plane_index)
        try:
            scan, arr, px_x_m, px_y_m = load_scan_plane(self.path, self.plane_index)
            display_arr = dataset_builder_display_array(arr, flatten=self.flatten_enabled)
            result = LoadedSampleRaw(
                key=key,
                scan=scan,
                arr=arr,
                px_x_m=px_x_m,
                px_y_m=px_y_m,
                flatten_enabled=self.flatten_enabled,
                display_arr=display_arr,
            )
        except Exception as exc:
            _log.warning("DatasetBuilderSampleLoadWorker: failed to load %s (%s)", self.path, exc)
            self.signals.failed.emit(key, str(exc), self.token)
            return
        self.signals.loaded.emit(key, result, self.token)


class QuickSegPrepSignals(QObject):
    finished = Signal(object, object, object)  # key, prepared, token
    failed = Signal(object, str, object)       # key, message, token


class QuickSegPrepWorker(_PooledWorker):
    def __init__(self, key, arr, params, px_x_m, px_y_m, token) -> None:
        super().__init__(QuickSegPrepSignals())
        self.key = key
        self.arr = arr
        self.params = params
        self.px_x_m = float(px_x_m)
        self.px_y_m = float(px_y_m)
        self.token = token

    def work(self):
        from probeflow.dataset_builder.quickseg import prepare_quickseg_inputs

        try:
            prepared = prepare_quickseg_inputs(
                self.arr,
                self.params,
                pixel_size_x_m=self.px_x_m,
                pixel_size_y_m=self.px_y_m,
            )
        except Exception as exc:
            _log.warning("QuickSegPrepWorker: failed for %s (%s)", self.key, exc)
            self.signals.failed.emit(self.key, str(exc), self.token)
            return
        self.signals.finished.emit(self.key, prepared, self.token)


class QuickSegWatershedSignals(QObject):
    finished = Signal(object, object, object)  # key, labels, token
    failed = Signal(object, str, object)       # key, message, token


class QuickSegWatershedWorker(_PooledWorker):
    def __init__(self, key, prepared, seeds, params, token) -> None:
        super().__init__(QuickSegWatershedSignals())
        self.key = key
    ...
    def work(self):
        from probeflow.dataset_builder.quickseg import watershed_labels

        try:
            labels = watershed_labels(self.prepared, self.seeds, self.params)
        except Exception as exc:
            _log.warning("QuickSegWatershedWorker: failed for %s (%s)", self.key, exc)
            self.signals.failed.emit(self.key, str(exc), self.token)
            return
        self.signals.finished.emit(self.key, labels, self.token)


class DatasetBuilderExportSignals(QObject):
    finished = Signal(object)
    failed = Signal(str)


class DatasetBuilderExportWorker(_PooledWorker):
    def __init__(self, spec) -> None:
        super().__init__(DatasetBuilderExportSignals())
        self.spec = spec
    ...
    def work(self):
        ...
        try:
            summary = export_dataset(self.spec)
        except Exception as exc:
            _log.warning("DatasetBuilderExportWorker: export failed (%s)", exc)
